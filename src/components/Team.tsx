...
const Team = () => {
  ...
  const advisors = [{
    ...
    role: "WHO Digital Health Advisor"
  }, {
    name: "Mark Stevens",
    role: "Former CEO, Anthem"
  }];
  return (
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 mb-10 sm:mb-16">
          {team.map((member, index) => (
            <div
              key={index}
              className="group p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-card border border-border/50 hover:border-primary/30 hover:shadow-xl transition-all duration-300 text-center"
            >
              <div
                className={`w-16 h-16 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br ${member.gradient} flex items-center justify-center text-white text-lg sm:text-2xl font-bold mx-auto mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300`}
              >
                {member.image}
              </div>
        ...
        <div className="bg-muted/30 rounded-xl sm:rounded-2xl p-5 sm:p-8">
          <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Advisory Board</h3>
