...
const ProblemSolution = () => {
  ...
  return (
    <section className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="hidden sm:block absolute bottom-0 right-1/4 w-96 h-96 bg-secondary/10 rounded-full blur-3xl" />
      ...
      <div className="container mx-auto max-w-7xl relative">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 mb-16 sm:mb-24">
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="inline-flex items-center gap-2 sm:gap-2.5 mb-6 sm:mb-8 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-destructive/10 border border-destructive/30">
              ...
              <span className="text-xs sm:text-sm font-bold text-destructive">Current Problems</span>
            </div>
            <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 sm:mb-10 leading-tight">Healthcare Data is Broken</h3>
            <div className="space-y-3 sm:space-y-4">
              {problems.map((problem, index) => (
                <div
                  key={index}
                  className="group flex gap-3 sm:gap-5 p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-card/50 backdrop-blur-sm border border-border hover:border-destructive/50 transition-all duration-300 hover:shadow-lg"
                >
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 sm:mb-10 leading-tight">Patient Bio Changes Everything</h3>
            <div className="space-y-3 sm:space-y-4">
              {solutions.map((solution, index) => (
                <div
                  key={index}
                  className="group relative flex gap-3 sm:gap-5 p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-card/50 backdrop-blur-sm border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-xl overflow-hidden"
                >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-8">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="group relative text-center p-6 sm:p-10 rounded-2xl sm:rounded-3xl bg-card/50 backdrop-blur-sm border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-2xl overflow-hidden"
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${stat.gradient} opacity-0 group-hover:opacity-10 transition-opacity`} />
              <div className="relative">
                <div className={`inline-flex w-12 h-12 sm:w-16 sm:h-16 mb-4 sm:mb-6 rounded-xl sm:rounded-2xl bg-gradient-to-br ${stat.gradient} items-center justify-center shadow-lg`}>
                  <stat.icon className="h-6 w-6 sm:h-8 sm:w-8 text-white" />
                </div>
                <div className={`text-4xl sm:text-5xl lg:text-6xl font-extrabold mb-2 sm:mb-4 bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent`}>
                  {stat.value}
                </div>
                <div className="text-base sm:text-xl font-bold mb-2 sm:mb-3 text-foreground">{stat.label}</div>
                <div className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{stat.subtext}</div>
              </div>
            </div>
          ))}
        </div>
